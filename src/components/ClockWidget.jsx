...
// ============================================================
// CLOCK MODE (Isolated Render Cycle)
// ============================================================
function ClockMode() {
...
// ============================================================
// STOPWATCH MODE (Isolated Render Cycle)
// ============================================================
function StopwatchMode() {
  ...
  useEffect(() => {
    let interval;
    if (swRunning) {
      // Note: For true production, we'd use requestAnimationFrame and delta time to prevent drift,
      // but this isolated setInterval is perfectly fine for a UI widget.
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-3.5 mt-1" onPointerDown={(e) => e.stopPropagation()}>
        <button
...
// ============================================================
// TIMER MODE (Isolated Render Cycle)
// ============================================================
function TimerMode() {
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between w-full mt-1" onPointerDown={(e) => e.stopPropagation()}>
...
export default function ClockWidget({ constraintsRef, zIndex, onFocus }) {
  ...
  return (
    <motion.div
      ...
      className="
        custom-widget absolute top-14 right-6 w-[280px]
        bg-[var(--color-surface)]/80 backdrop-blur-2xl
        border border-[var(--color-surface-border)] rounded-[var(--radius-window)]
        p-4.5 flex flex-col font-primary min-h-[175px]
        select-none pointer-events-auto cursor-grab popover-shadow transition-colors duration-250
      "
    >
      {/* Tab Select Toolbar */}
      <div className="flex gap-4 items-center px-1 mb-4" onPointerDown={(e) => e.stopPropagation()}>
        {['clock', 'stopwatch', 'timer'].map((tab) => (
          <button
            key={tab}
            onClick={() => setMode(tab)}
            className={`
              text-[11px] font-medium capitalize transition-colors duration-150 cursor-default focus:outline-none
              ${mode === tab ? 'text-[var(--color-accent)] font-semibold' : 'text-[var(--color-text-tertiary)] hover:text-[var(--color-text-secondary)]'}
            `}
          >
            {tab}
          </button>
