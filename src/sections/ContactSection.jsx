    ...
    <div className="w-full min-h-full bg-surface text-white flex flex-col font-primary selection:bg-accent/30 selection:text-white">
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar p-6 md:p-8 space-y-6 max-w-2xl w-full mx-auto">
        ...
        <div className="space-y-2">
          <h2 className="text-[11px] font-semibold text-white/40 uppercase tracking-wider px-1">
            Communications
          </h2>
          <div className="bg-[#202022]/40 rounded-xl border border-white/5 overflow-hidden shadow-sm divide-y divide-white/5">
            <ContactCard
              icon={<FiMail size={15} />}
              label="Email"
      ...
      {/* Footer */}
      <div className="p-3 text-center text-[11px] font-normal text-white/20 border-t border-white/5 bg-[#1A1A1C]/30 shrink-0 select-none">
...
function ContactCard({ icon, label, value, href }) {
  ...
  const content = (
    <div className="w-full px-5 py-4 flex items-center justify-between group bg-transparent hover:bg-white/[0.02] transition-colors duration-150">
      <div className="flex items-center gap-4 min-w-0">
        <div className="text-white/40 group-hover:text-accent transition-colors duration-150 shrink-0">
          {icon}
        </div>
        ...
        <div className="flex flex-col min-w-0">
          ...
          <span className="text-[14px] font-medium text-white/80 group-hover:text-white transition-colors duration-150 truncate mt-0.5">
            {value}
          </span>
        </div>
      </div>
      ...
      {isLink && (
        <div className="text-white/10 group-hover:text-white/40 group-hover:translate-x-0.5 transform transition-all duration-150 pr-1 shrink-0">
